...
<html>
<head>
	<title>Modifica indirizzi di fatturazione</title>
	<?php	
		include 'links.html';
	?>
...
<?php	
	...
	if(!$connessione->getNumRows())
		extR("	<div style = 'display: flex; justify-content: center; margin-top: 25px; width:100%'>
					non hai nessun indirizzo di fatturazione.
				</div>","myaccount.php");

	while($connessione->fetch())
	{
		echo "	<form id = 'Container' action = 'EditFattAddrForm.php' method = 'POST'>
						<div id = 'Indirizzo'>
							<div class = 'NomeCampo'>
								<p>Nome e Cognome:</p>
			...
			echo "					
								...
								if(!empty($connessione->get('PartitaIVA')))
									echo "<p>{$connessione->get('PartitaIVA')}</p>";
			echo "					
							</div>
						</div>
